...
const MAX_UPLOAD_SIZE = 5 * FILE_SIZE; // 5MB maximale Upload-GrÃ¶ÃŸe
...
// Routen fuer den ersten Testserver (eigener Server)
...
app.post('/upload', (req, res) => {
    ...
    req.on('data', chunk => {
        ...
        if (receivedSize > MAX_UPLOAD_SIZE) {
            ...
            res.status(413).send('Datei zu groÃŸ');
            return;
        }
    });
...
io.on('connection', (socket) => {
    ...
    // Event-Handler fuer den ersten Testserver (eigener Server)
    socket.on('ping', (data) => {
    ...
    socket.on('upload_end', () => {
        ...
        // Ergebnisse speichern (nur fuer den Testserver)
        const results = {
            ClientId: clientId,
            IP: clientIP,
            Port: clientPort,
            Ping: averagePing !== null ? averagePing.toFixed(2) + ' ms' : 'nicht gemessen',
            Jitter: jitter !== null ? jitter.toFixed(2) + ' ms' : 'nicht gemessen',
